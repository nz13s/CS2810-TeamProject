 ...
 *
 * @author Jatin Khatra, Anas Choudhury, Bhavik Narang, Oliver Graham
 */
public class Categories {
  ...
  public Categories(Connection connection) throws SQLException {
    ...
    fetchMenu = connection.prepareStatement(
            "SELECT food_id, food_name, food_description, calories, price, available, "
                    + "category, c.category_id, image_url "
                    + "FROM food "
  ...
  public Menu fetchMenu() throws SQLException {
    ...
    while (resultSet.next()) {
      ...
      Category c = categories.stream().filter(cat -> cat.getCategoryNumber()
              == food.getCategoryID()).findFirst().orElse(
