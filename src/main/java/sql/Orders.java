 ...
 *
 * @author Tony Delchev, Bhavik Narang, Jatin Khatra
 */
public class Orders {
  ...
  public Orders(Connection connection) throws SQLException {
    orderById = connection.prepareStatement(
            "SELECT order_id, time_ordered, order_confirmed, order_preparing, "
                    + "order_ready, order_served, table_num "
                    + "FROM orders "
                    + "WHERE order_id = ?");

    orderSave = connection.prepareStatement(
            "INSERT INTO orders (table_num, time_ordered, order_confirmed, "
    ...
    ordersGet = connection.prepareStatement(
            "SELECT o.*, i.food_id, i.order_id, f.food_name, f.food_description, f.calories, "
                    + "f.category_id,  i.quantity FROM orders AS o "
    ...
    ordersGetUnconfirmed = connection.prepareStatement(
            "SELECT o.*, i.food_id, i.order_id, f.food_name, f.food_description, "
                    + "f.calories, f.category_id,  i.quantity FROM orders AS o  "
                    + "JOIN food_orders AS i ON o.order_id = i.order_id "
                    + "JOIN food f on i.food_id = f.food_id "
    ...
    getAllCancelledOrders = connection.prepareStatement(
            "SELECT food.food_id, food.food_name, food.price, food.available, "
                    + "orders.order_id, orders.table_num, food_orders.quantity "
                    + "FROM food_orders "
  ...
  public ArrayList<Order> getCancelledOrders() throws SQLException {
    ...
    while (set.next()) {
      ...
      if (cancelledOrders.containsKey(orderID)) {
        cancelledOrders.get(orderID).addFoodItem(new Item(food, quantity));
      } else {
        ArrayList<Item> items = new ArrayList<>();
        items.add(new Item(food, quantity));
  ...
  public ArrayList<Order> getOrdersUnconfirmed(int tableNum) throws SQLException {
    ...
    while (resultSet.next()) {
      Food food = new Food(resultSet.getInt("food_id"),
              ...
              resultSet.getString("food_description"),
              resultSet.getInt("calories"),
              null, resultSet.getInt("category_id"),
              null, null);
      int orderID = resultSet.getInt("order_id");
      if (orders.containsKey(orderID)) {
        orders.get(orderID).addFoodItem(new Item(food, resultSet.getInt("quantity")));
  ...
  /**
   * Set time confirmed and order id to the orderConfirm query and execute.
   *
   * @param orderID of the order to confirm
